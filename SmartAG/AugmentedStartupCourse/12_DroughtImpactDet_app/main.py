...
class DroughtDataRequest(BaseModel):
    PS: float
    QV2M: float
    T2M: float
    T2MDEW: float
...
def predict_drought(features: DroughtDataRequest):
    ...
    # Return the predicted potability as 0 to 5 
    return output
...
async def predict(request: Request, json_data: str = Form(...)):
    ...
    prediction = predict_drought(DroughtDataRequest(**features))
